"""Offline checks for the composability manifest (``src/anthropic_handlers/catalog.yaml``).

Asserts the manifest loads, every workflow carries a non-empty intent summary +
tags + param_schema, every listed qualified_name is plausibly real (its leaf
name is declared as a `workflow`/`facet` in the package's .ffl sources, and each
facet namespace is a declared `namespace`), and there are no duplicate entries.
...
from anthropic_handlers import catalog

# Repo root: tests/ -> repo root is parents[1].
_REPO_ROOT = Path(__file__).resolve().parents[1]
_FFL_ROOT = _REPO_ROOT / "src" / "anthropic_handlers"


def _all_ffl_text() -> str:
    parts = [p.read_text(encoding="utf-8") for p in _FFL_ROOT.rglob("*.ffl")]
    assert parts, "no .ffl sources found under src/anthropic_handlers"
    return "\n".join(parts)
...
def test_manifest_loads(manifest):
    ...
    assert manifest.get("package") == "fwh-anthropic"
...
def test_workflows_have_summary_and_tags():
    for wf in catalog.workflows():
        ...
        assert wf["param_schema"], f"empty param_schema for {qn}"
...
def test_manifest_effect_cost_match_ffl_mixins(ffl_text):
    """The effect/cost in the manifest must match the `with Effect/Cost` mixins in the FFL.

    For each cataloged facet, find its declaration line in the FFL and assert the
    same kind/tier appear on (or near) it — guards against the manifest drifting
    from the annotations.
    """
    for fc in catalog.facets():
        leaf = fc["qualified_name"].rsplit(".", 1)[-1]
        # Grab the facet declaration through its closing `)` + any trailing mixins,
        # which (per the annotation style) sit on the `=> (...)` return line.
        m = re.search(
            rf"\bfacet\s+{re.escape(leaf)}\b.*?=>\s*\([^)]*\)([^\n]*)",
            ffl_text,
            re.DOTALL,
        )
        assert m, f"could not locate facet declaration for {leaf}"
        tail = m.group(1)
        assert f'Effect(kind = "{fc["effect"]}")' in tail, (
            f"{leaf}: manifest effect={fc['effect']} not on FFL line: {tail!r}"
        )
        assert f'Cost(tier = "{fc["cost"]}")' in tail, (
            f"{leaf}: manifest cost={fc['cost']} not on FFL line: {tail!r}"
        )
