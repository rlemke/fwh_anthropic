"""ComputerUse event-facet handlers.

Status: scaffolded. _DISPATCH is empty; register_handlers is
a no-op. Wire facets in by:

1. Adding pure-function wrappers to anthropic_handlers.tools._lib.computer_use
2. Importing them here via the shim
3. Adding entries to _DISPATCH mapping facet names to handlers
4. The standard `handle` / `register_handlers` / `register_computer_use_handlers`
